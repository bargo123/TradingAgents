...
- Provider failures leave terminal evidence untouched and leave unresolved horizons pending; terminal evaluation rows are immutable.
...
- [ ] **Step 1: Write failing store tests.** Assert table creation on a Phase 4 database, nullable `training_eligible`, the basis check constraint, triple-key uniqueness (both bases at the same horizon coexist; duplicate reruns do not duplicate), status-aware updates from PENDING only, immutable COMPLETE/DATA_UNAVAILABLE/INELIGIBLE rows, and migration of an older evaluation table missing optional columns. Assert that unavailable/ineligible rows may have null target/entry/future fields while complete rows retain all quote/calculation fields.
...
- [ ] **Step 3: Implement the SQLite store.** Create `shadow_decision_evaluations` with the spec's checks, nullable fields, and triple primary key. Add idempotent column migration for an existing evaluation table. Serialize UTC timestamps with the repository's `Z` convention and nullable numeric/text values. Use one transaction for batches and `ON CONFLICT ... DO UPDATE` guarded by `evaluation_status = 'PENDING'`; never overwrite terminal evidence. Keep legacy `shadow_decisions.future_evaluation_status` untouched.
...
- **Spec coverage:** Tasks 1–2 cover temporal decision/reference capture and the read-only provider seam; Tasks 3–5 cover bases, eligibility, maturity, tolerance, formulas, HOLD, MFE/MAE, storage, idempotency, batch reuse, metrics, and zero LLM calls; Task 6 covers the separate CLI and docs; Task 7 covers guarded real-terminal/safety proof; Task 8 covers the required final evidence and baseline-scoped diff.
