...
class ExperienceQueryService:
    def __init__(self, records: Sequence[Any] | Any, index: ExactSimilarityIndex | None = None, profile=None,
        ...
        if hasattr(records, "active_records"):
            self.catalog = records; records = tuple(records.active_records()) + tuple(records.historical_records())
        else:
            self.catalog = None
        ...
        self.index = index or ExactSimilarityIndex(vectors, {str(_get(r, "experience_id")): r for r in self.records})

    def _profile(self, record, query):
        if self.profile is not None: return self.profile
    ...
    def _cohort(row):
        c = _get(row, "cohort")
        if c is not None: return c if isinstance(c, NormalizationCohortV1) else NormalizationCohortV1(*tuple(c))
        return NormalizationCohortV1(str(_get(row, "symbol")), str(_get(row, "analysis_profile")), str(_get(row, "analysis_timeframe")), str(_get(row, "feature_schema_version", "")), str(_get(row, "feature_extractor_version", "")))
    ...
    def search(self, query: ExperienceQuery) -> ExperienceSearchResult:
        ...
        for row in rows:
            ...
            tombstoned = bool(_get(row, "tombstoned", False)) or (bool(aliases) and not any(str(v) == "CURRENT" or getattr(v, "value", None) == "CURRENT" for v in aliases.values()))
            if query.symbol and _get(row, "symbol", _get(row, "resolved_symbol")) != query.symbol: reason = "symbol"
            elif query.analysis_profile and _get(row, "analysis_profile") != query.analysis_profile: reason = "profile"
        ...
        if profile is None: return ExperienceSearchResult(candidate_count=len(rows), excluded_counts=exclusions, active_generation_id=self.generation_id)
