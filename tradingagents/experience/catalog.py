...
from .provenance import validate_evaluation_provenance
...
class ExperienceCatalog:
    ...
    def upsert_source_alias(
        ...
        with self._connect() as db:
            ...
            if not existing:
                ...
                db.execute(
                    "INSERT INTO experience_decision_evidence VALUES (?,?)",
                    (experience_id, _json(kwargs.get("decision_evidence"))),
                )
                db.execute(
                    "INSERT INTO experience_market_provenance VALUES (?,?)",
                    (experience_id, _json(kwargs.get("provenance"))),
                )
            db.execute(
                "INSERT OR REPLACE INTO experience_source_aliases VALUES (?,?,?,?,?,?,?)",
                (source_database_id, decision_id, fingerprint, experience_id, "CURRENT", now, "OK"),
            )
            db.execute(
                ...
                ("ALIAS_ACCEPTED", source_database_id, decision_id, experience_id, "{}", now),
            )
            db.execute(
                "UPDATE experience_records SET tombstoned=0 WHERE experience_id=?", (experience_id,)
            )
    ...
    def _record(self, db: sqlite3.Connection, experience_id: str) -> ExperienceRecord:
        ...
        aliases = {
            f"{r['source_database_id']}:{r['source_decision_id']}": r["state"]
            for r in db.execute(
        ...
        return ExperienceRecord(
            ...
            decision_completed_timestamp=datetime.fromisoformat(row["decision_completed_timestamp"])
            if row["decision_completed_timestamp"]
            else None,
            decision_reference_timestamp=datetime.fromisoformat(row["decision_reference_timestamp"])
            ...
            source_evaluation_fingerprints=json.loads(row["source_evaluation_fingerprints_json"]),
        )
    ...
    def current_alias_count(self, experience_id: str) -> int:
        with self._connect() as db:
            return int(
                db.execute(
                    "SELECT COUNT(*) FROM experience_source_aliases WHERE experience_id=? AND state='CURRENT'",
                    (experience_id,),
                ).fetchone()[0]
            )
    ...
    def mark_alias_removed(
        ...
        with self._connect() as db:
            row = db.execute(
                "SELECT experience_id FROM experience_source_aliases WHERE source_database_id=? AND source_decision_id=? AND accepted_fingerprint=?",
                (source_database_id, decision_id, fingerprint),
            ).fetchone()
            if not row:
                return
            db.execute(
                "UPDATE experience_source_aliases SET state='REMOVED', scan_status='SOURCE_MISSING', observed_at=? WHERE source_database_id=? AND source_decision_id=?",
                (_now(), source_database_id, decision_id),
            )
            if not db.execute(
                "SELECT 1 FROM experience_source_aliases WHERE experience_id=? AND state='CURRENT'",
                (row[0],),
            ).fetchone():
                db.execute(
                    "UPDATE experience_records SET tombstoned=1 WHERE experience_id=?", (row[0],)
                )

    def mark_last_alias_removed(self, experience_id: str) -> None:
        with self._connect() as db:
            row = db.execute(
                "SELECT source_database_id,source_decision_id,accepted_fingerprint FROM experience_source_aliases WHERE experience_id=? AND state='CURRENT' ORDER BY source_database_id LIMIT 1",
                (experience_id,),
            ).fetchone()
        if row:
            self.mark_alias_removed(*row)

    def append_evaluation_snapshot(
        self,
        experience_id: str,
        ...
        **kwargs: Any,
        ...
        if "evaluation_fingerprint" in prov and prov["evaluation_fingerprint"] != fingerprint:
            ...
            raise ProvenanceViolationError(
                "provenance evaluation fingerprint does not match snapshot fingerprint"
            )
        ...
        with self._connect() as db:
            ...
            db.execute(
                "INSERT OR IGNORE INTO experience_outcome_snapshots VALUES (?,?,?,?,?,?,?)",
                (
                    experience_id,
                    evaluation.get("evaluation_basis"),
                    evaluation.get("horizon_seconds"),
                    fingerprint,
                    _json(evaluation),
                    _json(prov),
                    _now(),
    ...
    def evaluation_snapshot_fingerprints(self, experience_id: str) -> tuple[str, ...]:
    ...
    def record_failed_scan(self, source_database_id: str, reason: str, detail: Any = "") -> None:
        with self._connect() as db:
            ...
            db.execute(
                "INSERT INTO experience_import_events(event_type,source_database_id,detail_json,observed_at) VALUES (?,?,?,?)",
                ("FAILED_SCAN", source_database_id, _json({"reason": reason}), _now()),
            )
            db.execute(
                "INSERT INTO experience_diagnostic_fts(label,source_database_id) VALUES (?,?)",
                (label, source_database_id),
            )
    ...
    def _quarantine_db(self, db, source_database_id, decision_id, fingerprint, reason, detail):
        ...
        db.execute(
            "INSERT INTO experience_diagnostic_fts(label,source_database_id,decision_id) VALUES (?,?,?)",
            (diagnostic_label(reason), source_database_id, decision_id),
        )
    ...
    def quarantine_count(self) -> int:
        with self._connect() as db:
            return int(db.execute("SELECT COUNT(*) FROM experience_quarantine").fetchone()[0])
