...
def _timestamp(value: Any) -> datetime:
    try:
        ...
        raise FeatureExtractionIncompleteError(
            "TEMPORAL_INVALID:analysis_snapshot_timestamp"
        ) from None
...
def extract_market_state(decision_row: Mapping[str, Any] | Any) -> MarketStateVector:
    ...
    row = (
        ...
        if isinstance(decision_row, Mapping)
            ...
            {f.name: getattr(decision_row, f.name) for f in fields(decision_row)}
            if is_dataclass(decision_row)
            else vars(decision_row)
        )
    )
    ...
    if not isinstance(symbol, str) or not symbol.strip():
        diags.append(ExtractionDiagnostic("IDENTITY_MISSING", "resolved_symbol", "required"))
    ...
    if dt is None:
        diags.append(
            ExtractionDiagnostic("TEMPORAL_INVALID", "analysis_snapshot_timestamp", "required")
        )
    ...
    if not isinstance(raw, Mapping):
        diags.append(ExtractionDiagnostic("SCHEMA_INVALID", "snapshot_json", "mapping required"))
        raw = {}
    quote = raw.get("quote")
    if not isinstance(quote, Mapping):
        diags.append(
            ExtractionDiagnostic("SCHEMA_INVALID", "snapshot_json.quote", "mapping required")
        )
        quote = {}
    ...
    try:
        valid_digits = (
            not isinstance(digits, bool)
            and isinstance(digits, (int, float))
            and math.isfinite(float(digits))
            and int(digits) == digits
            and 0 <= int(digits) <= 10
        )
    ...
    if not valid_digits:
        diags.append(
            ExtractionDiagnostic("DIGITS_INVALID", "snapshot_json.digits", "integer required")
        )
    _fail(diags)
    values: list[float] = []
    mask: list[bool] = []
    ...
    def add(name: str, value: Any, path: str, *, direction: bool = False) -> None:
        if direction:
            if value is None or str(value).upper() == "INSUFFICIENT_DATA":
            ...
            if encoded is None:
                diags.append(
                    ExtractionDiagnostic(
                        "DIRECTION_INVALID", path, "expected UP, FLAT, DOWN, or INSUFFICIENT_DATA"
                    )
                )
                values.append(float("nan"))
                mask.append(False)
            ...
            if original is not None and value is None:
                diags.append(
                    ExtractionDiagnostic("VALUE_NON_FINITE", path, "finite numeric value required")
                )
        if value is None:
            values.append(float("nan"))
            mask.append(False)
    ...
    add(
        ...
        if row.get("analysis_snapshot_spread_points") is not None
        else "snapshot_json.quote.spread_points",
    )
    features = raw.get("features", {})
    if not isinstance(features, Mapping):
        diags.append(
            ExtractionDiagnostic("SCHEMA_INVALID", "snapshot_json.features", "mapping required")
        )
        features = {}
    for tf in ("M1", "M5", "M15", "H1"):
        ...
        if section_value != {} and not isinstance(section_value, Mapping):
            diags.append(
                ExtractionDiagnostic(
                    "SCHEMA_INVALID", f"snapshot_json.features.{tf}", "mapping required"
                )
            )
        ...
        add(
            ...
            f"snapshot_json.features.{tf}.direction",
            direction=True,
        )
