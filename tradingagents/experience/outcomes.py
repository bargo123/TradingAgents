...
class OutcomeStatsCalculator:
            ...
            if isinstance(snapshots, Mapping):
                ...
                for key, value in snapshots.items():
                    ...
                    if isinstance(key, tuple) and len(key) == 2:
                        ...
                        continue
                    normalized.append(dict(value, evaluation_basis=basis, horizon_seconds=int(horizon)))
                snapshots = tuple(normalized)
        return tuple(snapshots or ())

    @staticmethod
    def _availability(snapshot: Mapping[str, Any]) -> datetime | None:
        return _utc(snapshot.get("recovered_from_unavailable_at")) or _utc(snapshot.get("evaluated_at")) or _utc(snapshot.get("created_at"))
    ...
    def _select(self, snapshots: tuple[dict[str, Any], ...], request: OutcomeStatsRequest) -> tuple[dict[str, Any] | None, str | None]:
        exact = [s for s in snapshots if s.get("evaluation_basis") == request.evaluation_basis and int(s.get("horizon_seconds") or 0) == request.horizon_seconds]
        if not exact:
        ...
        for snapshot in exact:
            ...
            if observation is not None and observation <= request.as_of and available is not None and available <= request.as_of:
        ...
        if any(s.get("evaluation_status") == "DATA_UNAVAILABLE" and _utc(s.get("observation_timestamp")) is not None and _utc(s.get("observation_timestamp")) <= request.as_of and self._availability(s) is not None and self._availability(s) <= request.as_of for s in exact):
            prior = [s for s in exact if s.get("evaluation_status") == "DATA_UNAVAILABLE" and _utc(s.get("observation_timestamp")) <= request.as_of and self._availability(s) <= request.as_of]
    ...
    def calculate(self, request: OutcomeStatsRequest) -> OutcomeStatistics:
        ...
        for experience_id in request.experience_ids:
            ...
            if record is None:
                ...
                tier = TrustTier(_get(record, "trust", _get(record, "trust_tier", TrustTier.TIER_C_DIAGNOSTIC_ONLY)))
                ...
                if reason is None:
                    ...
                    elif request.evaluation_basis == "DECISION_REFERENCE" and str(snapshot.get("decision_reference_status", "AVAILABLE")).upper() != "AVAILABLE":
                        reason = "REFERENCE_UNAVAILABLE"
                    elif not all(_finite(snapshot.get(field)) for field in ("buy_net_points", "sell_net_points")):
                        reason = "REQUIRED_FIELD_MISSING_OR_NONFINITE"
                    elif str(snapshot.get("selected_action", "")).upper() == "HOLD" and not _finite(snapshot.get("hold_opportunity_cost_points")):
                        reason = "REQUIRED_FIELD_MISSING_OR_NONFINITE"
                if reason is None:
                    eligible_count += 1
