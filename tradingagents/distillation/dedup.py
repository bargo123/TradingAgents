...
def canonical_example_id(example: Any) -> str:
    text = repr(example) if not isinstance(example, dict) else "|".join(f"{k}={example[k]}" for k in sorted(example))
    return hashlib.sha256(_norm(text).encode()).hexdigest()
@dataclass
class DedupIndex:
    near_threshold: float = .90
    ...
    def check(self, example: Any) -> str | None:
        ...
        if key in self._keys: return "DUPLICATE"
        tokens = set(text.split())
        for prior in self._tokens:
            score = len(tokens & prior) / max(1, len(tokens | prior))
            if score >= self.near_threshold: return "NEAR_DUPLICATE"
        self._keys.add(key); self._tokens.append(tokens)
        return None
