...
COHORT = NormalizationCohortV1(
    "EURUSD", "INTRADAY", "M15", "experience-features.v1", "phase8-feature-extractor.v1"
)
...
def test_unrelated_rows_do_not_change_exact_cohort_profile():
    ...
    unrelated_symbol = row(
        ...
        cohort=NormalizationCohortV1(
            ...
            "INTRADAY",
            "M15",
            COHORT.feature_schema_version,
            COHORT.feature_extractor_version,
        ),
    )
    unrelated_timeframe = row(
        999.0,
        when,
        cohort=NormalizationCohortV1(
            "EURUSD", "SWING", "H1", COHORT.feature_schema_version, COHORT.feature_extractor_version
        ),
    )
    base = build_profile(base_rows, COHORT, (TrustTier.TIER_A_HIGH_TRUST,), None)
    expanded = build_profile(
        base_rows + [unrelated_symbol, unrelated_timeframe],
        COHORT,
...
def test_tier_a_only_changes_population_fingerprint():
    when = datetime(2026, 1, 1, tzinfo=UTC)
    rows = [
        row(1.0, when, tier=TrustTier.TIER_A_HIGH_TRUST),
        row(2.0, datetime(2026, 1, 1, 1, tzinfo=UTC), tier=TrustTier.TIER_B_LIMITED),
    ]
...
def test_historical_profile_excludes_future_rows_and_removed_current_aliases():
    ...
    before = [
        row(1.0, datetime(2026, 1, 1, tzinfo=UTC), completion=datetime(2026, 1, 1, 1, tzinfo=UTC))
    ]
    future = row(
        9999.0, datetime(2026, 1, 3, tzinfo=UTC), completion=datetime(2026, 1, 3, 1, tzinfo=UTC)
    )
...
def test_query_fingerprint_contains_policy_and_cutoff():
    ...
    fingerprint = query_normalization_fingerprint(
        profile, COHORT, (TrustTier.TIER_A_HIGH_TRUST,), None
    )
