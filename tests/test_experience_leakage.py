...
def _row(
    ...
    *,
    ...
    action: str = "BUY",
    outcome: float = 1.0,
    cohort: NormalizationCohortV1 = COHORT,
    tier: TrustTier = TrustTier.TIER_A_HIGH_TRUST,
    current: bool = True,
) -> dict:
    ...
    return {
        ...
        "decision_completed_timestamp": completed or when + timedelta(minutes=1),
        "trust": tier,
        "trust_tier": tier,
        "source_aliases": {"db:d": "CURRENT" if current else "REMOVED"},
        "values": tuple(value + i * 0.01 for i in range(8)),
        "mask": (True,) * 8,
        "feature_names": NAMES,
        "cohort": cohort,
...
def test_future_rows_do_not_change_historical_normalization():
    ...
    assert (
        build_profile(before, COHORT, (TrustTier.TIER_A_HIGH_TRUST,), cutoff).to_fingerprint()
        == build_profile(
            before + [future], COHORT, (TrustTier.TIER_A_HIGH_TRUST,), cutoff
        ).to_fingerprint()
    )
...
def test_evaluation_future_row_is_not_visible_before_observation():
    snapshot = {
        ...
        "observation_timestamp": "2026-01-03T12:00:00Z",
        "evaluated_at": "2026-01-03T13:00:00Z",
        "selected_action": "BUY",
        "buy_net_points": 2.0,
        "sell_net_points": -1.0,
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                ...
                violations.update(
                    name
                    for name in names
                    if any(word in name.lower().split(".") for word in forbidden)
                )
