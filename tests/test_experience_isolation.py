...
def row(
    eid, *, cohort=COHORT, tier=TrustTier.TIER_A_HIGH_TRUST, current=True, when=None, completed=None
):
    when = when or datetime(2026, 1, 1, tzinfo=UTC)
    return {
        "experience_id": eid,
        "symbol": cohort.resolved_symbol,
        "analysis_profile": cohort.analysis_profile,
        "analysis_timeframe": cohort.analysis_timeframe,
        "analysis_snapshot_timestamp": when,
        "decision_completed_timestamp": completed or when,
        "trust": tier,
        "trust_tier": tier,
        "source_aliases": {"db:d": "CURRENT" if current else "REMOVED"},
        "values": (0.0,) * 8,
        "mask": (True,) * 8,
        "feature_names": NAMES,
        "cohort": cohort,
...
def test_symbol_filter_is_independent():
    unrelated = NormalizationCohortV1(
        "XAUUSD", "P", "M15", COHORT.feature_schema_version, COHORT.feature_extractor_version
    )
...
def test_profile_filter_is_independent():
    other = NormalizationCohortV1(
        "EURUSD", "SWING", "M15", COHORT.feature_schema_version, COHORT.feature_extractor_version
    )
...
def test_timeframe_filter_is_independent():
    other = NormalizationCohortV1(
        "EURUSD", "P", "H1", COHORT.feature_schema_version, COHORT.feature_extractor_version
    )
    result = _service([row("good"), row("other", cohort=other)]).search(
        query(analysis_timeframe="M15")
    )
    assert [hit.experience_id for hit in result.hits] == ["good"]
...
def test_tier_c_normalization_is_rejected():
    with pytest.raises(ValueError, match="Tier C"):
        build_profile(
            [row("c", tier=TrustTier.TIER_C_DIAGNOSTIC_ONLY)],
            COHORT,
...
def test_recovered_evaluation_without_prior_snapshot_is_not_fabricated():
    record = {
        ...
        "outcome_snapshots": (
            _complete("COMPLETE", "2026-01-03T12:00:00Z", "2026-01-03T13:00:00Z", "late"),
        ),
    }
    result = OutcomeStatsCalculator([record]).calculate(
        OutcomeStatsRequest(("late",), horizon_seconds=300, as_of=datetime(2026, 1, 2, tzinfo=UTC))
    )
...
def test_recovered_evaluation_with_prior_unavailable_snapshot_preserves_history():
    ...
    result = OutcomeStatsCalculator([record]).calculate(
        OutcomeStatsRequest(
            ("recovered",), horizon_seconds=300, as_of=datetime(2026, 1, 2, 14, tzinfo=UTC)
        )
    )
