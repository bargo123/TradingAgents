...
def test_tier_c_numeric_similarity_is_rejected():
    with pytest.raises(ValueError, match="Tier C"):
        ExperienceQueryService([row("c", tier=TrustTier.TIER_C_DIAGNOSTIC_ONLY)], profile=profile()).search(
            ExperienceQuery(query_state(), trust_tiers=(TrustTier.TIER_C_DIAGNOSTIC_ONLY,)))
...
def test_action_filter_is_applied_after_identity_gates_without_changing_scores():
    ...
    assert filtered.hits[0].distance == next(h.distance for h in unfiltered.hits if h.experience_id == "buy")
...
def test_query_market_state_cohort_must_match_selected_profile():
    ...
    result = service.search(ExperienceQuery(query_state(cohort=NormalizationCohortV1("XAUUSD", "P", "M15", "experience-features.v1", "extractor"))))
    assert result.hits == ()
