...
def test_distill_without_teacher_fails_closed(capsys, tmp_path):
    ...
    plan.write_text(json.dumps({"generation_id": "g", "packets": [], "request_fingerprint": "f"}), encoding="utf-8")
...
def test_inspect_does_not_require_teacher(capsys, tmp_path):
    ...
    generation.mkdir()
    (generation / "manifest.json").write_text(json.dumps({"generation_id": "generation-x", "status": "EMPTY", "counts": {}}), encoding="utf-8")
