...
def profile():
    return SimilarityProfileV1(
        medians={n: 0.0 for n in NAMES}, iqr={n: 1.0 for n in NAMES},
        mad_scales={n: 1.0 for n in NAMES}, scales={n: 1.0 for n in NAMES},
        fallback_scales={n: 1.0 for n in NAMES}, epsilon=1e-9,
        weights={n: 1.0 for n in NAMES}, clipping=(-8.0, 8.0),
        feature_order=NAMES, mask_policy="exclude-missing.v1",
        trust_tiers=(), cohort=COHORT,
    )
...
def test_search_uses_masked_float32_weighted_rms_and_deterministic_ties():
    ...
    }, metadata={"a": {"analysis_snapshot_timestamp": datetime(2026, 1, 1, tzinfo=UTC)},
