...
def _ensure_fresh(root: Path) -> None:
    if root.exists():
        ...
        raise ExperienceArtifactNotEmptyError(f"Phase 8 artifact parent does not exist: {root.parent}")
    root.mkdir(parents=False, exist_ok=False)


def run_smoke(source_db: str | Path, *, experience_artifact_root: str | Path,
              knowledge_artifact_root: str | Path | None,
              knowledge_embedding_model_path: str | Path | None, offline: bool = False) -> dict[str, Any]:
    if not offline:
    ...
    os.environ.update({"KNOWLEDGE_OFFLINE": "1", "HF_HUB_OFFLINE": "1", "TRANSFORMERS_OFFLINE": "1"})
    ...
    with guard:
        ...
        for record in records:
            try:
                vector = extract_market_state(next(r for r in snapshot.decisions if str(r["decision_id"]) == record.source_decision_id))
                vectors[record.experience_id] = vector
            except Exception:
                continue
        feature_rows = {
            record.experience_id: {
                "experience_id": record.experience_id, "values": vector.values,
                "mask": vector.mask, "feature_names": vector.feature_names,
                "cohort": vector.cohort, "feature_fingerprint": vector.fingerprint,
                "trust_tier": record.trust, "trust": record.trust,
            } for record in records if (vector := vectors.get(record.experience_id))
        }
        profiles = {}
        for cohort_values in {tuple(v.cohort) for v in vectors.values()}:
            ...
            profiles[cohort] = build_profile(rows, cohort, (TrustTier.TIER_A_HIGH_TRUST, TrustTier.TIER_B_LIMITED)) if rows else None
        experience_service = ExperienceQueryService(records, feature_vectors=feature_rows, profiles={k: v for k, v in profiles.items() if v}, generation_id=generation.generation_id)
        ...
        try:
            ...
            pass
        config = KnowledgeConfig(source_root=config_source_root, artifact_root=knowledge_root, embedding_model_path=model_path, offline=True)
        ...
        knowledge_service = KnowledgeQueryService(vector_reader, lexical_reader, knowledge_catalog, embedder)
        orchestrator = EvidenceOrchestrator(knowledge_service, experience_service, stats)
        market_state = next(iter(vectors.values()), None)
        state = {"values": list(market_state.values), "mask": list(market_state.mask), "feature_names": list(market_state.feature_names), "cohort": list(market_state.cohort)} if market_state else {"values": [], "mask": []}
        bundle = orchestrator.query(EvidenceRequest(research_question="market risk and execution", market_state=state, knowledge_top_k=3, experience_top_k=3))
    after = _source_integrity(source)
    report = {
        "source_path": str(source), "source_hash_before": before["db"]["sha256"], "source_hash_after": after["db"]["sha256"],
        ...
        "analysis_invocations": 0, "network_attempts": guard.attempt_count, "bounded_latency_ms": round((time.monotonic() - started) * 1000, 2),
        "fresh_experience_artifact_root": str(root), "diagnostics": [e.message for e in bundle.errors],
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        print(json.dumps(run_smoke(args.source_db, experience_artifact_root=args.experience_artifact_root, knowledge_artifact_root=args.knowledge_artifact_root, knowledge_embedding_model_path=args.knowledge_embedding_model_path, offline=args.offline), sort_keys=True, default=str, indent=2))
        return 0
